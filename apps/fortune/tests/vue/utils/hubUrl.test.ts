/**
 * mirrors apps/fortune/src/utils/hubUrl.ts
 */
...
describe('resolveHubUrl', () => {
  ...
  it('vite dev server (5175) からはハブ入口ページのポート 5173 を指す', () => {
    expect(resolveHubUrl({ protocol: 'http:', hostname: 'localhost', port: '5175' })).toBe(
      'http://localhost:5173/',
    )
  })

  it('vite preview (4175) からはハブ入口ページのポート 4173 を指す', () => {
    expect(resolveHubUrl({ protocol: 'http:', hostname: '100.93.52.49', port: '4175' })).toBe(
