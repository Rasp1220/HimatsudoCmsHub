...
各CMS側にもハブへ戻るリンクがあり、ログイン後はサイドバー上部の
「← 統合CMSハブへ戻る」、ログイン画面ではフォーム下部のリンクから
入口ページへ戻れます（CMSを切り替えたいときに使います）。

戻り先のURLは `apps/*/src/utils/hubUrl.ts` が自動判定します。

| アクセスしているURL | 戻り先 |
|---|---|
| 本番（`admin.himatsudo.com/himatsudo/` など） | 同一オリジンの `/` |
| 開発時の Vite dev server（ポート 5174 / 5175） | 同じホストの 5173 |
| 自宅サーバの `vite preview`（ポート 4174 / 4175） | 同じホストの 4173 |

ハブを別ドメインで運用しているなど自動判定が合わない場合は、
`apps/*/.env` の `VITE_HUB_URL` で明示的に指定できます。
